...
`src/eugene_plexus_inference/_generated/`.
...
GENERATED_DIR = REPO_ROOT / "src" / "eugene_plexus_inference" / "_generated"
WORKING_DIR = REPO_ROOT / ".codegen-cache"

SPECS_TARBALL_URL_TEMPLATE = "https://github.com/eugene-plexus/specs/archive/{ref}.tar.gz"

# inference.yaml hosts the inference component's path operations and its
# own `InferenceEndpoint` schema, but inlines most request/response bodies
# under `paths` and $refs the shared schemas (Message, Health, Config*,
# Problem, Checkpoint, …) into components/common.yaml, which is a complete
# OpenAPI document (with `paths: {}`) hosting them. We generate from both:
# common.yaml -> common_models.py for the shared schemas, and
# inference.yaml -> models.py for the path-local inline shapes and
# InferenceEndpoint. The resulting modules include a handful of types the
# inference component doesn't strictly need — that's fine, they're cheap
# and keep the generator's job simple.
SPECS_TO_GENERATE = [
    ("openapi/inference.yaml", "models.py"),
...
def run_codegen(specs_root: Path, ref: str) -> None:
    ...
    for input_rel, output_name in SPECS_TO_GENERATE:
        ...
        cmd = [
            ...
            "--disable-timestamp",
            # inference.yaml defines its request/response shapes inline under
            # `paths` (its only `components.schemas` entry is
            # InferenceEndpoint; the rest live in common.yaml). Without
            # `paths` scope the generator finds almost nothing in
            # inference.yaml. Mirrors connector's/trainer's codegen, which
            # feed a paths-bearing spec the same way.
            "--openapi-scopes",
            "paths",
            "schemas",
        ]

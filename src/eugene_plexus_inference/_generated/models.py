...
class InferenceEndpoint(BaseModel):
    """
    v0.3-draft. One served model: a checkpoint (+ optional adapter) exposed
    under an OpenAI-compatible model id. Referenced from
    `common.yaml#/components/schemas/ExportSettings.target`.

    """

    endpointId: UUID
    name: str = Field(
        ..., description='OpenAI-compatible model id clients pass as `model`.'
    )
    checkpointId: UUID | None = None
    adapterCheckpointId: UUID | None = None
...
class V1ChatCompletionsPostResponse(BaseModel):
    pass
...
class V1ModelsGetResponse(BaseModel):
    object: str | None = 'list'
    data: list[Datum] | None = None


class V1InferenceEndpointsGetResponse(BaseModel):
    endpoints: list[InferenceEndpoint]


class V1InferenceEndpointsEndpointIdLoadPostRequest(BaseModel):
    checkpointId: UUID
    adapterCheckpointId: UUID | None = Field(
        None,
        description='Optional LoRA/QLoRA adapter to load on top of the base checkpoint.',
    )
...
class V1ChatCompletionsPostRequest(BaseModel):
    model: str = Field(..., description='Endpoint / served-checkpoint id.')
    messages: list[Message]
